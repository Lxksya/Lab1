...
namespace GeneticsProject
{
    ...
    class Program
    {
        ...
        static void ProcessCommands(string filepath, StreamWriter writer)
        {
            using (var reader = new StreamReader(filepath))
            {
                ...
                while ((command = reader.ReadLine()) != null)
                {
                    ...
                    {
                        case "search":
                            writer.WriteLine($"{commandNumber:D3} search {RLDecoding(args[1])}");
        ...
        static string RLDecoding(string formula)
        {
        ...
        {
            string expandedSequence = RLDecoding(aminoSequence);
        ...
        {
            string formula1 = RLDecoding(RetrieveFormula(protein1));
            string formula2 = RLDecoding(RetrieveFormula(protein2));
        ...
        static void Main(string[] args)
        {
            ...
            {
                writer.WriteLine("Frolov Georgiy");
                writer.WriteLine("Genetic Searching");
                writer.WriteLine(new string('-', 74));

                LoadProteinData("sequences2.txt");

                ProcessCommands("commands2.txt", writer);
            }
